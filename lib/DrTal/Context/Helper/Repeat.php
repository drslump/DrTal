...
namespace DrTal::Context::Helper;

class Repeat implements Iterator
{
    private $key;
    private $current;
    private $valid;
 ...
 */
class DrTal_Context_Helper_Repeat_Groups {
    ...
    public function __get( $var )
    {
        ...
        //$value = DrTal_Context::resolve( $this->data, $var );
        $base = $this->data;
        if ( is_array($base) ) {
            $base = new ArrayObject($base);
        }
        
        if ( !is_object($base) ) {
            $value = null;
        } else if ( property_exists($base, $var) ) {
            $value = $base->$var;
        } else if ( is_callable( array($base, $var), false, $callable ) ) {
            $value = $base->$var();
        } else {
            $value = null;
        }        
