...
require_once DRTAL_INCLUDE_BASE . 'DrTal/Parser/Exception.php';
require_once DRTAL_INCLUDE_BASE . 'DrTal/Parser/Writer.php';
require_once DRTAL_INCLUDE_BASE . 'DrTal/Parser/Util/StringStream.php';
...
/*
 ...
 Notes:
    ...
    XMLReader class constants are available since PHP 5.1
*/
class Parser
{
    ...
    /*
     ...
     Arguments:
        $template   - a <DrTal::Template> object
    */
    ...
    */
    ...
    /*
     ...
     Throws:
        - <DrTal_Parser_Exception> if $nsObj does not inherit from <DrTal::Parser::Namespace>
    */
    public function registerNamespace( DrTal::Parser::Generator::Base::Ns $nsObj, $uri = null, $prefix = null )
    {
        if ( !$uri ) {
    ...
    */
    ...
    /*
     ...
     Throws:
        - <DrTal_Parser_Exception> if $filterObj does not inherit from <DrTal_Parser_Filter>
    */
    public function registerFilter( $name, $filterObj )
    {
        if ( !($filterObj instanceof DrTal_Parser_Filter) ) {
            throw new DrTal_Parser_Exception( get_class($filterObj) . ' must inherit from DrTal_Parser_Filter' );
        }
    ...
    */    
    ...
    /*
     ...
     Throws:
        - <DrTal_Parser_Exception> if $callback is not actually callable
    */    
    public function registerTales( $name, $callback )
    {
        if ( !is_callable($callback) ) {
            throw new DrTal::Parser::Exception( "The callback supplied for the '$name' tales handler is not valid" );
        }
    ...
    */
    ...
    /*
     ...
     Throws:
        - <DrTal_Parser_Exception> if there was an error generating the template
    */
    public function build()
    {
        ...
        $this->codegen = new DrTal_Parser_Codegen( $this->template );
        ...
        if ( DrTal::debugging() ) {
            $this->codegen
            ->php('$ctx->setDebugTemplate(\'' . "\n    " . implode("\n    ", str_split(base64_encode(gzdeflate($tpl,9)), 76)) . '\');')->EOL();
        }
        ...
        if ( DrTal::debugging() ) {
            foreach ( $this->namespaces as $uri => $ns ) {
                $this->codegen->php('$ctx->setDebugNamespace(\'' . $uri . '\', \'' . $ns['prefix'] . '\');')->EOL();
            }
        }
        ...
        } catch ( DrTal_Parser_Exception $e ) {
        ...
        if ( !empty($errors) ) {
            ...
            $exc = new DrTal_Parser_Xml_Exception( 'Error parsing template' );
        ...
        $this->codegen->append('<?php /* Generation took: ' . (microtime(true)-$startTime) . ' seconds */ ?>');
        ...
        do {
            ...
            switch ( $reader->nodeType ) {
                case XMLReader::ELEMENT:
                    ...
                    $debug = '<' . $reader->name;
                    if ( DrTal::debugging() ) {
                        $this->getCodegen()->php('$ctx->setDebugHint(\'' . $reader->name . '\');')->EOL();
                    }
                    ...
                    if ( $nsObj->hasElement($name) ) {
                        ...
                        if ( !class_exists($class) ) {
                            throw new DrTal_Parser_Exception( "Element handler class '$class' not found" );
                        }
                        $elmObj = new $class( $this, $nsObj, $reader->name, $isEmpty );
                    } else {
                        throw new DrTal_Parser_Exception( "'($ns) $name' has no handler" );
                    }
                    ...
                    if ( $reader->hasAttributes ) {
                        ...
                        while ($reader->moveToNextAttribute()) {
                            ...
                            if ( !$nsObj->hasAttribute( $reader->localName ) ) {
                                throw new Exception("Attribute '{$reader->localName}' has no handler");
                            }
                        ...
                        $attrs = DrTal::sortByPriority( $attrs );
                        
                        // Attach the attributes to the element
                        foreach ( $attrs as $attr ) {
                            if ( !class_exists($attr['class']) ) {
                                throw new DrTal_Parser_Exception( "Attribute handler class '{$attr['class']}' not found" );
                            }
                ...
                case XMLReader::PI:
                    ...
                    if ( $data !== null && $data !== false ) {
                        $this->codegen->xml( '<?' . $reader->name . ' ' . $data . '?>' );
                    }
                ...
                case XMLReader::ENTITY_REF:
                    ...
                    if ( $data === null || $data === false ) {
                        ...
                        $this->codegen->xml( $val );
                    } else {
                        $this->codegen->xml( '&' . $reader->localName . ';' );
                    }
                break;
